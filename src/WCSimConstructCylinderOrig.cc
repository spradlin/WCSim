...
G4LogicalVolume* WCSimDetectorConstruction::ConstructCylinderOrig()
{
	G4cout << "**** Building Cylindrical Detector with original version! ****" << G4endl;
  ...
  if(isODConstructed) {
    ...
    //G4LogicalSkinSurface *TyvekCaveBarrelSurface =i
...
// These lines add color to the blacksheet in the extratower. If using RayTracer, comment the first chunk and use the second. The Blacksheet should be green.
  ...
  G4LogicalVolume* logicWCPMT = ConstructPMT(WCPMTName, WCIDCollectionName, "tank",false);
...
If used here, uncomment the SetVisAttributes(WClogic) line, and comment out the SetVisAttributes(G4VisAttributes::Invisible) line.*/
  ...
  G4LogicalVolume* logicTopCapAssembly = ConstructCapsOrig(-1);
  G4LogicalVolume* logicBottomCapAssembly = ConstructCapsOrig(1);
...
G4LogicalVolume* WCSimDetectorConstruction::ConstructCapsOrig(G4int zflip)
{
